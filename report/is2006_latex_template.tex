...
Kubernetes like Docker is an open source solution. While Swarm provides an initial attempt at cluster orchestration, Kubernetes provides orchestration management and a monitoring tool for larger clusters. Kubernetes is built by the same team of engineers who developed the Google compute platform and extends on the experience those engineers have from managing and tooling internal clusters \citep{Brewer}. Google has been running container based clusters in production for a number of years. Many of these developers were working on Kubernetes processor Borg. Borg's primary role is to manage the lifecycles of tasks and machines \citep{Verma}. Kubernetes is a scheduler similar to Mesos\footnote{http://mesos.apache.org/}, a resource scheduler which is used for monitoring and automated deployment.
\par
...
While monitoring the integrity of a live cluster replication controllers may be called upon to manage the infrastructure of the application depending on the manifest. Replication Controllers serve as a template manifest to the dynamic allocate resources that are available to the cluster and the initial creation and monitoring of these resources is maintain via replication controller yaml file.  Using this file you can specify the number of replicas that you want a pod to have. If there are too many pods the status quo will be restore the the amount of pods defined using the ?Äìreplica tag. The same will hold if there are too few and more pods shall be created.  The replication controller maintains the desired number of pods matches its label selector are operational.

\subsection{Labels}
Labels are in effect a naming convention that is assigned to pods for management of the computation resources. This naming schema may be used to roll out a testing/development application or a full production implementation depending on the use case. Labeling gives clients the ability to easily access the current resources that are available scaling up or down as the need requires. Unlike a naming convection using ID?s labels do not provide uniqueness, many objects can contain the same label but kubernetes labels define a taxonomy of resources that are available.

\subsection{Network Services}
Network services provide the communication of not just pods but the communication between clusters. Communication within a Kubernetes cluster happens through a rigorous API that is accessed via the kubectl tool and kube-proxy. On node initialize kubectl is placed on every node while kube-proxy facilitates a kubernetes service abstraction by maintaining network rules on the host and performing connection forwarding. Kubelet manages pods, their children, images and volumes. Persistent volumes have been some what of an issue for Swarm based solutions. This has only been addressed in version 1.9.
\par 
How does Kubernetes compare to Borg or even Swarm? As Kubernette is a descendant from Borg there is overlap in many design decisions that were taken. Borg groups work by jobs while Kubernetes adds the concept of labels for greater flexibility. Also Borg uses an IP per machine, Kubernetes uses a network-per-machine and IP per pod. Borg's API is complex requiring a steep learning curve; Kubernetes APIs are is simpler but still more complex than Docker Swarms implementation.
\par
Kubernetes builds on these concepts to make dependency management easier to solve. Dependencies between containers can be managed by keeping them in a pod, a pod represents a boundary. By assigning an ip address to a pod, port conflicts are virtually extinguished completely. Typically in container applications there may be conflict over ports, previous to Kubernetes, one approach to mitigate against conflicts was by dynamically allocating port numbers  - this type of approach makes it difficult to implement DNS directly on a cluster reducing flexibility.
\par
In addition to the pod containing an IP address a service is also represented by an IP address as follows. A service is indefinite a persistent entity, that is it is represented by a long-lived name, the name can be a DNS name where this DNS can map to an IP address. Every service in effect has its own load balancer. So if a request is sent to the IP address, the service request can be distributed among the pods. To access a pool of services all that is required is the DNS of the service and not the member. This is a very important concept where the pods and the underlining infrastructure are not exposed to the client.
